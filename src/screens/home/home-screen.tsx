...
export default function HomeScreen() {
  ...
  useEffect(() => {
    ...
    const refreshWeather = () => fetchWeather(deviceLocation);
    refreshWeather();
  ...
  async function fetchWeather(location: DeviceLocation) {
    try {
      setIsLoadingWeather(true);
      setWeatherError(null);

      const weatherResponse = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${location.latitude}&longitude=${location.longitude}&current=temperature_2m,weather_code,wind_speed_10m,relative_humidity_2m&timezone=auto`,
      );
      ...
      setWeather({
        ...
        city: locationCity ?? "Ubicación desconocida",
  ...
  async function handleGetWeather() {
    const locationReady = deviceLocation ?? (await handleGetLocation(false));
    if (locationReady) {
      await fetchWeather(locationReady);
    }
  }

  async function handleGetLocation(
    showAlert = true,
  ): Promise<DeviceLocation | false> {
    try {
      ...
      if (!(await Location.hasServicesEnabledAsync())) {
        ...
        if (showAlert) {
          Alert.alert(
            "Activa la ubicación",
            "Activa manualmente la ubicación del dispositivo para consultar el clima y la cercanía.",
          );
        }
        return false;
      }
      ...
      if (permission.status !== "granted") {
        ...
        if (showAlert) {
          Alert.alert(
            ...
            "Permite el acceso a la ubicación para consultar el clima y la cercanía.",
          );
        }
        return false;
      }
      ...
      const geoAddress = await Location.reverseGeocodeAsync(nextLocation);
      setLocationCity(
        geoAddress[0]?.city ??
      ...
      setLocationCountry(geoAddress[0]?.country ?? "");
      ...
      {activeTab === "funciones" ? (
        ...
        >
          ...
          >
            ...
            {deviceLocation ? (
              <>
                <Text style={{ color: "#E6F7FF", marginTop: 16 }}>
